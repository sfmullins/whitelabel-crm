...
router.post('/email-drafts',(req,res,next)=>{try{res.status(201).json(service.createDraft(parse(draftCreate,req.body) as any));}catch(error){forward(next,error);}});
...
router.get('/workflow-studio',(req,res,next)=>{try{const query=parse(z.object({includeArchived:z.preprocess((value)=>value==='true'||value===true,z.boolean().default(false))}).strict(),req.query);res.json(service.listWorkflows(Boolean(query.includeArchived)));}catch(error){forward(next,error);}});

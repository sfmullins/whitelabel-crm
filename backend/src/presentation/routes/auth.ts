...
router.post('/auth/login',(req,res,next)=>{
  try{
    ...
    res.json(result);
  }catch(error){res.status(401).json({error:'INVALID_CREDENTIALS',message:error instanceof Error?error.message:'Invalid credentials'});}
});
...
router.post('/auth/password',(req:CrmRequest,res)=>{const input=parse(z.object({password:z.string().min(12)}).strict(),req.body);security.setPassword(String(req.crm?.identity?.id),input.password);res.json({updated:true});});

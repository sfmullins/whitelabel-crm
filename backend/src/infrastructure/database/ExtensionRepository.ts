...
export class ExtensionRepository {
  ...
  async install(packageInput:unknown,context:InstallContext){
    ...
    try{
      ...
      this.connection.transaction(()=>{
        ...
        const bindings=this.connection.prepare(`SELECT contribution_type,contribution_key FROM extension_bindings WHERE extension_id=?`).all(extensionId) as Array<{contribution_type:string;contribution_key:string}>;const disable=this.connection.prepare(`UPDATE extension_bindings SET disabled_at=? WHERE extension_id=? AND contribution_type=? AND contribution_key=?`);
        for(const binding of bindings)if(!activeBindingKeys.has(`${binding.contribution_type}|${binding.contribution_key}`))disable.run(installedAt,extensionId,binding.contribution_type,binding.contribution_key);
  ...
  setEnabled(id:string,enabled:boolean){
    ...
    this.connection.transaction(()=>{
      ...
      this.connection.prepare(`UPDATE extension_bindings SET disabled_at=? WHERE extension_id=?`).run(enabled?null:now,id);
    })();
  ...
  exportExtension(id:string){
    ...
    return {formatVersion:1,exportedAt:timestamp(),extension,attempts,migrations:migrations.map((item)=>({...item,operation_json:parseJson(item.operation_json,{}),rollback_json:parseJson(item.rollback_json,null)}))};
  }

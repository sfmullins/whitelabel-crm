...
export class ActivityRepository implements IActivityRepository {
  ...
  async create(input: ActivityPersistenceCreate): Promise<Activity> {
    ...
    this.connection.prepare(`
      insert into activities (
        ...
        @occurredAt, @followUpDate, @source, @sourceReference,
    ...
    if (!created) {
      throw new Error('Activity was not found after create');
    }
    return created;
  }
  ...
  async list(options: ActivityListOptions): Promise<Activity[]> {
    ...
    if (options.contactId) {
      predicates.push('contact_id = @contactId');
      params.contactId = options.contactId;
    }
    if (options.engagementId) {
      predicates.push('engagement_id = @engagementId');
      params.engagementId = options.engagementId;
    }
    if (options.type) {
      predicates.push('type = @type');
      params.type = options.type;
    }
    if (options.occurredFrom) {
      predicates.push('occurred_at >= @occurredFrom');
      params.occurredFrom = options.occurredFrom;
    }
    if (options.occurredTo) {
      predicates.push('occurred_at <= @occurredTo');
      params.occurredTo = options.occurredTo;
    }
    if (options.followUpFrom) {
      predicates.push('follow_up_date >= @followUpFrom');
      params.followUpFrom = options.followUpFrom;
    }
    if (options.followUpTo) {
      predicates.push('follow_up_date <= @followUpTo');
      params.followUpTo = options.followUpTo;
    }
  ...
  async update(id: string, patch: ActivityUpdate): Promise<Activity | null> {
    ...
    if ('contactId' in patch) {
      assignments.push('contact_id = @contactId');
      params.contactId = patch.contactId ?? null;
    }
    if ('engagementId' in patch) {
      assignments.push('engagement_id = @engagementId');
      params.engagementId = patch.engagementId ?? null;
    }
    ...
    if (patch.body !== undefined) {
      assignments.push('body = @body');
      params.body = patch.body.trim();
    }
    if (patch.author !== undefined) {
      assignments.push('author = @author');
      params.author = patch.author.trim();
    }
    if (patch.occurredAt !== undefined) {
      assignments.push('occurred_at = @occurredAt');
    ...
    return result.changes === 1
      ? this.getById(id, { includeArchived: true })
      : null;
  }

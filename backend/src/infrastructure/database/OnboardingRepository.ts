...
export class OnboardingRepository {
  ...
  evaluate(value:unknown):ReadinessResult{
    ...
    add({id:'identity.complete',category:'identity',status:identityComplete?'passed':'failed',severity:'required',title:'Business identity',explanation:identityComplete?'The business identity and primary contact details are complete.':'Business name, email, phone and address are required.',remediation:'Complete the Business identity section.',section:'identity',evidence:{displayName:Boolean(identity?.displayName?.trim()),email:Boolean(identity?.email?.includes('@')),phone:Boolean(identity?.phone?.trim()),address:Boolean(identity?.address?.trim())}});
    const businessProfileConfirmed=configuration.businessProfile?.confirmed===true;
    add({id:'business-profile.confirmed',category:'business-model',status:businessProfileConfirmed?'passed':'failed',severity:'required',title:'Business model confirmed',explanation:businessProfileConfirmed?'The owner confirmed how the business works.':'The suggested business model has not been confirmed by the owner.',remediation:'Review how the business sells and serves customers, then confirm the answers.',section:'business-fit',evidence:{confirmed:businessProfileConfirmed}});
    ...
    const enabledExtensions=configuration.extensions?.filter((item)=>item.enabled).map((item)=>item.packageKey)??[];const unavailable=enabledExtensions.filter((key)=>!this.connection.prepare(`SELECT 1 FROM extensions WHERE package_key=? AND status='enabled'`).get(key));

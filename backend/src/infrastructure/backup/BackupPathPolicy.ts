...
function isContained(root:string,target:string):boolean {const relative=path.relative(root,target);return relative===''||(!relative.startsWith('..')&&!path.isAbsolute(relative));}
function existingRealPath(candidate:string):string {try{return fs.realpathSync.native(candidate);}catch{return candidate;}}
...
export function resolveBackupPath(rootDirectory:string,filename:string):string {
  ...
  const root=path.resolve(rootDirectory);const target=path.resolve(root,filename);
  if(!isContained(root,target))throw new Error('Backup path escapes the configured backup directory');
  const realRoot=existingRealPath(root);const realTarget=existingRealPath(target);
  if(!isContained(realRoot,realTarget))throw new Error('Backup path escapes the configured backup directory');
  return target;
}

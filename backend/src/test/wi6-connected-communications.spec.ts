...
import { serializeEmailSyncCursor } from '../infrastructure/integrations/ConnectedAdapters';
...
const calendarAdapter:CalendarSyncAdapter={
  async test(){},
  async discover(){return [{providerCalendarKey:'https://dav.example/calendars/primary/',displayName:'Primary',syncToken:'token-0'}];},
  async fetchSince(){return {nextCursor:'token-1',deletedResourceHrefs:[],events:[{providerEventKey:'meeting-acme',resourceHref:'https://dav.example/calendars/primary/resource-17.ics',title:'Acme leadership review',description:'Review findings.',startsAt:'2026-07-22T09:00:00.000Z',endsAt:'2026-07-22T10:00:00.000Z',timezone:'Europe/Dublin',attendees:[{name:'Aisling Byrne',address:'aisling.byrne@acme.example'}],cancelled:false}]};},
};
...
describe('WI6 connected communications',()=>{
  ...
  it('discovers CalDAV calendars, stores exact hrefs and processes due reminders once',async()=>{
    ...
    const account=service.createAccount({kind:'calendar',name:'Good Order calendar',serverUrl:'https://dav.example/calendars/',username:'consultant@goodorder.example',password:'secret',settings:{}});const result=await service.syncAccount(String(account.id));expect(result.status).toBe('succeeded');
  ...
  it('queues a durable notification before marking a reminder delivered',async()=>{
    const work=new WorkRepository();for(const existing of work.listReminders({status:'pending',dueOnly:true}))work.updateReminderStatus(String(existing.id),'cancelled');
    const reminder=work.createReminder({sourceType:'task',sourceId:'20000000-0000-4000-8000-000000000011',organisationId:ACME,scheduledAt:'2020-01-01T00:00:00.000Z'});

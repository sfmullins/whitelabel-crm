...
describe('backup archive cryptography v2',()=>{
  ...
  beforeEach(()=>{
    fixture=fs.mkdtempSync(path.join(os.tmpdir(),'crm-backup-crypto-'));
    databasePath=path.join(fixture,'source.db');archivePath=path.join(fixture,'archive.crmbackup');restoredPath=path.join(fixture,'restored.db');
    const database=new Database(databasePath);database.exec("CREATE TABLE settings(id TEXT PRIMARY KEY); INSERT INTO settings(id) VALUES ('default');");database.close();
  });
  afterEach(()=>fs.rmSync(fixture,{recursive:true,force:true}));

  const manifest=(overrides:Partial<BackupManifest>={}):BackupManifest=>({
    formatVersion:1,applicationVersion:'1.0.0',schemaVersion:1,createdAt:new Date().toISOString(),deviceId:'test-device',databaseFile:'source.db',databaseSizeBytes:fs.statSync(databasePath).size,sha256:BackupManager.calculateSHA256(databasePath),encrypted:true,...overrides,
  });

  it('uses a unique salted PBKDF2 work factor and authenticates the manifest',()=>{
    const password='correct horse battery staple';const first=BackupManager.createPasswordKdf(password);const second=BackupManager.createPasswordKdf(password);
    expect(first.kdf.saltHex).not.toBe(second.kdf.saltHex);
    const metadata=manifest({kdf:first.kdf});fs.writeFileSync(archivePath,BackupManager.encryptBackupFile(databasePath,metadata,first.key));
  ...
  it('continues to decrypt legacy version 1 archives',()=>{
    const key=crypto.randomBytes(32);const metadata=manifest();const manifestBuffer=Buffer.from(JSON.stringify(metadata));const nonce=crypto.randomBytes(12);const cipher=crypto.createCipheriv('aes-256-gcm',key,nonce);const ciphertext=Buffer.concat([cipher.update(fs.readFileSync(databasePath)),cipher.final()]);const version=Buffer.alloc(2);version.writeUInt16BE(1,0);const length=Buffer.alloc(4);length.writeUInt32BE(manifestBuffer.length,0);fs.writeFileSync(archivePath,Buffer.concat([Buffer.from('CRMB'),version,nonce,cipher.getAuthTag(),length,manifestBuffer,ciphertext]));

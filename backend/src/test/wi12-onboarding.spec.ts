...
describe('WI12 instance onboarding and deployment profiles',()=>{
  ...
  it('validates, backs up, signs and atomically publishes a secret-free profile',async()=>{
    ...
    const published=await onboarding.publish(LOCAL_OWNER_USER_ID);expect(published.prePublicationBackupCreated).toBeUndefined();expect(published.backupPath).toContain('pre-publication-test.db');expect(published.workspace.instance.status).toBe('active');expect(published.workspace.published?.state).toBe('published');expect(published.workspace.draft.revision).toBe(2);

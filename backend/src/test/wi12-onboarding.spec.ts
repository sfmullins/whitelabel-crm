...
describe('WI12 instance onboarding and deployment profiles',()=>{
  ...
  it('creates a resumable draft, preserves WI10 indexes and grants bounded permissions',()=>{
    const workspace=repository().getWorkspace();expect(workspace.instance.status).toBe('provisioning');expect(workspace.draft.state).toBe('draft');expect(workspace.readiness.publishable).toBe(false);expect(workspace.deploymentProfileAvailable).toBe(false);
    ...
    const indexes=(getSqliteConnection().prepare(`SELECT name FROM sqlite_master WHERE type='index'`).all() as Array<{name:string}>).map((row)=>row.name);
    expect(indexes).toEqual(expect.arrayContaining(['webhook_subscription_owner_idx','platform_event_aggregate_idx','webhook_delivery_subscription_idx']));
    const managerPlatformRead=(getSqliteConnection().prepare(`SELECT count(*) AS count FROM role_permissions rp JOIN roles r ON r.id=rp.role_id WHERE r.key='manager' AND rp.permission_key='platform.read'`).get() as {count:number}).count;
    expect(managerPlatformRead).toBe(1);
  });
  ...
  it('rejects stale draft saves and publication attempts',async()=>{
    ...
    await expect(onboarding.publish(LOCAL_OWNER_USER_ID,initial.draft.checksum)).rejects.toThrow('changed');
    const published=await onboarding.publish(LOCAL_OWNER_USER_ID,first.draft.checksum);expect(published.deploymentProfile.profile.businessIdentity.displayName).toBe('First Identity');
  });

  it('validates, backs up, signs and atomically publishes a secret-free profile',async()=>{
    const onboarding=repository();const configuration=validConfiguration();const saved=onboarding.saveDraft(configuration,LOCAL_OWNER_USER_ID);const readiness=onboarding.validateDraft(LOCAL_OWNER_USER_ID,saved.draft.checksum);expect(readiness.publishable).toBe(true);expect(readiness.failures).toBe(0);
    const published=await onboarding.publish(LOCAL_OWNER_USER_ID,saved.draft.checksum);expect(published.backupPath).toContain('pre-publication-test.db');expect(published.workspace.instance.status).toBe('active');expect(published.workspace.published?.state).toBe('published');expect(published.workspace.draft.revision).toBe(2);
  ...
  it('rejects profile tampering and creates a new immutable publication for rollback',async()=>{
    const onboarding=repository();let workspace=onboarding.saveDraft(validConfiguration('First Identity'),LOCAL_OWNER_USER_ID);const first=await onboarding.publish(LOCAL_OWNER_USER_ID,workspace.draft.checksum);const firstRevisionId=first.workspace.published!.id;
    workspace=onboarding.saveDraft(validConfiguration('Second Identity'),LOCAL_OWNER_USER_ID,first.workspace.draft.checksum);const second=await onboarding.publish(LOCAL_OWNER_USER_ID,workspace.draft.checksum);expect(second.deploymentProfile.profile.businessIdentity.displayName).toBe('Second Identity');
    ...
    const rolledBack=await onboarding.rollback(firstRevisionId,LOCAL_OWNER_USER_ID,second.workspace.draft.checksum);expect(rolledBack.deploymentProfile.profile.businessIdentity.displayName).toBe('First Identity');expect(rolledBack.deploymentProfile.profile.configurationRevision).toBeGreaterThan(second.deploymentProfile.profile.configurationRevision);
  });

  it('stores one-time enrolments as hashes and atomically binds a scoped user session to one device',async()=>{
    const onboarding=repository();const saved=onboarding.saveDraft(validConfiguration(),LOCAL_OWNER_USER_ID);await onboarding.publish(LOCAL_OWNER_USER_ID,saved.draft.checksum);const security=new SecurityRepository();const employee=security.createUser({email:'employee@northstar.example',displayName:'Northstar Employee',roleKeys:['member'],password:'a sufficiently long employee password'});
    ...
    const redemptionCount=(getSqliteConnection().prepare(`SELECT redeemed_count FROM instance_enrolments WHERE id=?`).get(issued.id) as {redeemed_count:number}).redeemed_count;expect(redemptionCount).toBe(1);
  ...
  it('keeps draft APIs private while allowing signed profile discovery and enrolment redemption',async()=>{
    ...
    try{
      ...
      const authorisedWorkspace=await authorised.json() as {draft:{checksum:string;configuration:OnboardingConfiguration}};
      const savedRoute=await fetch(`${server.url}/api/onboarding/draft`,{method:'PUT',headers:{authorization:`Bearer ${ownerSession.token}`,'content-type':'application/json'},body:JSON.stringify({configuration:authorisedWorkspace.draft.configuration,expectedChecksum:authorisedWorkspace.draft.checksum})});expect(savedRoute.status).toBe(200);
      const staleRoute=await fetch(`${server.url}/api/onboarding/draft`,{method:'PUT',headers:{authorization:`Bearer ${ownerSession.token}`,'content-type':'application/json'},body:JSON.stringify({configuration:authorisedWorkspace.draft.configuration,expectedChecksum:'0'.repeat(64)})});expect(staleRoute.status).toBe(409);

...
export class CommunicationsHubService {
  ...
  createReplyDraft(messageId:string,mode:'reply'|'reply_all'|'forward'){
    ...
    const originalSubject=String(row.subject??'');const prefix=mode==='forward'?'Fwd:':'Re:';const subject=/^(re|fwd):/i.test(originalSubject)?originalSubject:`${prefix} ${originalSubject}`.trim();
    const accountUsername=String(row.account_username??'').toLowerCase();
    let to:EmailAddress[]=[];let cc:EmailAddress[]=[];
    if(mode!=='forward'){
      if(direction==='outbound'){
        to=uniqueAddresses(recipients.to.filter((value)=>value.address.toLowerCase()!==accountUsername));
        if(mode==='reply_all')cc=uniqueAddresses(recipients.cc.filter((value)=>value.address.toLowerCase()!==accountUsername&&!to.some((target)=>target.address.toLowerCase()===value.address.toLowerCase())));
      }else{
        to=[from];
        if(mode==='reply_all')cc=uniqueAddresses([...recipients.to,...recipients.cc].filter((value)=>value.address.toLowerCase()!==accountUsername&&value.address.toLowerCase()!==from.address.toLowerCase()));
      }
    }
  ...
  async sendDraft(id:string,explicitConfirmation:boolean){
    ...
    const secret=this.vault.read(String(account.credentialKey));const original=draft.inReplyToMessageId?this.repository.getOriginalMessage(draft.inReplyToMessageId):null;
    const fromAddress=String(account.settings.fromAddress??account.username);const fromName=String(account.settings.fromName??'');const messageId=`${crypto.randomUUID()}@whitelabelcrm.local`;
    ...
    try{
      ...
      this.applyReconciliation(reconciliationId);this.repository.finishSendAttempt(attempt.id,{status:'succeeded',providerMessageKey:result.providerMessageKey,rfcMessageId:messageId});
      const stored=this.getDraft(id);this.triggers.trigger({triggerType:'email_sent',sourceType:'email_message',sourceId:stored.sentMessageId,eventId:stored.sentMessageId,context:{accountId:draft.accountId,organisationId:draft.organisationId,contactId:draft.contactId,engagementId:draft.engagementId,threadId:stored.threadId,subject:draft.subject}});
      return {...stored,accepted:result.accepted,rejected:result.rejected};
  ...
  async createCalendarEvent(input:{calendarId:string;event:CalendarWriteInput;organisationId?:string|null;contactId?:string|null;engagementId?:string|null}){
    ...
    try{const result=await this.calendarWriter.create(this.config(calendar),this.vault.read(String(calendar.credentialKey)),String(calendar.providerCalendarKey),input.event);transmitted=true;const reconciliationId=this.createReconciliation('calendar',operation.id,'create',{calendarId:input.calendarId,event:input.event,context:{organisationId:input.organisationId,contactId:input.contactId,engagementId:input.engagementId},result});const stored=this.applyReconciliation(reconciliationId) as any;this.repository.finishCalendarOperation(operation.id,{status:'succeeded',etag:result.etag,providerEventKey:result.providerEventKey});this.triggers.trigger({triggerType:'calendar_event_created',sourceType:'calendar_event',sourceId:String(stored.id),eventId:String(stored.id),context:{organisationId:stored.organisationId,contactId:stored.contactId,engagementId:stored.engagementId,title:stored.title,startsAt:stored.startsAt,endsAt:stored.endsAt}});return stored;}catch(error){this.finishCalendarFailure(operation.id,error,transmitted);throw new Error(transmitted?'Calendar event changed remotely but local reconciliation is required. Do not repeat the operation.':error instanceof Error?error.message:String(error));}
  }
  ...
  private applyReconciliation(id:string):unknown{const connection=getSqliteConnection();const row=connection.prepare('SELECT * FROM outbound_reconciliation_records WHERE id=?').get(id) as Record<string,unknown>|undefined;if(!row)throw new Error('Outbound reconciliation record not found');if(row.status==='reconciled')return null;const payload=parseJson<Record<string,any>>(row.payload_json,{});try{let result:unknown;if(row.kind==='email'){const draft=this.getDraft(String(payload.draftId));if(draft.status==='sent')result=draft;else result=this.repository.recordSentMessage(String(payload.draftId),{providerMessageKey:String(payload.providerMessageKey),rfcMessageId:String(payload.rfcMessageId),from:payload.from,sentAt:String(payload.sentAt)});}else if(row.operation==='create'){const existing=connection.prepare('SELECT id FROM calendar_events WHERE calendar_id=? AND provider_event_key=?').get(payload.calendarId,payload.result.providerEventKey) as {id:string}|undefined;result=existing?this.requireEvent(existing.id):this.repository.createLocalCalendarEvent(payload.calendarId,payload.event,{...payload.context,etag:payload.result.etag});const eventId=String((result as any).id);this.upsertCalendarResource(eventId,payload.calendarId,payload.result.resourceHref);}else{result=this.repository.updateLocalCalendarEvent(String(payload.eventId),payload.event,payload.result.etag);const current=this.requireEvent(String(payload.eventId));this.upsertCalendarResource(String(payload.eventId),String(current.calendarId),payload.result.resourceHref);}const timestamp=new Date().toISOString();connection.prepare("UPDATE outbound_reconciliation_records SET status='reconciled',error_summary=NULL,reconciled_at=?,updated_at=? WHERE id=?").run(timestamp,timestamp,id);return result;}catch(error){const message=error instanceof Error?error.message:String(error);connection.prepare("UPDATE outbound_reconciliation_records SET status='failed',error_summary=?,updated_at=? WHERE id=?").run(message,new Date().toISOString(),id);throw error;}}

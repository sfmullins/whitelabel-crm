...
export class CommunicationsHubService {
  ...
  async createCalendarEvent(input:{calendarId:string;event:CalendarWriteInput;organisationId?:string|null;contactId?:string|null;engagementId?:string|null}){
    ...
    try{const result=await this.calendarWriter.create(this.config(calendar),this.vault.read(String(calendar.credentialKey)),String(calendar.providerCalendarKey),input.event);const stored=this.repository.createLocalCalendarEvent(input.calendarId,input.event,{organisationId:input.organisationId,contactId:input.contactId,engagementId:input.engagementId,etag:result.etag});this.repository.finishCalendarOperation(operation.id,{status:'succeeded',etag:result.etag,providerEventKey:result.providerEventKey});this.triggers.trigger({triggerType:'calendar_event_created',sourceType:'calendar_event',sourceId:String(stored.id),eventId:String(stored.id),context:{organisationId:stored.organisationId,contactId:stored.contactId,engagementId:stored.engagementId,title:stored.title,startsAt:stored.startsAt,endsAt:stored.endsAt}});return stored;}catch(error){this.finishCalendarFailure(operation.id,error);throw error;}
  }

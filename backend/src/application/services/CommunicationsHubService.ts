...
export class CommunicationsHubService {
  ...
  async sendDraft(id:string,explicitConfirmation:boolean){
    ...
    const fromAddress=String(account.settings.fromAddress??account.username);const fromName=String(account.settings.fromName??'');const messageId=`${crypto.randomUUID()}@whitelabelcrm.local`;const sentAt=new Date().toISOString();
    const attachments=draft.documents.map((document:{documentId:string})=>{const content=this.documents.content(document.documentId);return {filename:content.filename,mimeType:content.mimeType,content:content.content};});
    const attempt=this.repository.startSendAttempt(id,draft.accountId);const reconciliationId=this.createReconciliation('email',id,'send',{draftId:id,providerMessageKey:messageId,rfcMessageId:messageId,from:{address:fromAddress,name:fromName||undefined},sentAt,remoteConfirmed:false});this.repository.markDraftSending(id);let transmitted=false;
    try{
      ...
      transmitted=true;this.confirmReconciliation(reconciliationId,{remoteConfirmed:true,providerMessageKey:result.providerMessageKey});this.applyReconciliation(reconciliationId);return {...this.getDraft(id),accepted:result.accepted,rejected:result.rejected};
    }catch(error){const message=error instanceof Error?error.message:String(error);if(!transmitted){this.removeReconciliation(reconciliationId);this.repository.markDraftFailed(id,message);}this.repository.finishSendAttempt(attempt.id,{status:'failed',rfcMessageId:messageId,errorSummary:transmitted?`Remote transmission succeeded; local reconciliation required: ${message}`:message});throw new Error(transmitted?'Email was transmitted but local reconciliation is required. Do not resend.':message);}
  }
  ...
  async createCalendarEvent(input:{calendarId:string;event:CalendarWriteInput;organisationId?:string|null;contactId?:string|null;engagementId?:string|null}){
    const calendar=this.requireCalendar(input.calendarId);const operation=this.repository.startCalendarOperation(input.calendarId,'create',null,null,input.event);const reconciliationId=this.createReconciliation('calendar',operation.id,'create',{calendarId:input.calendarId,event:input.event,context:{organisationId:input.organisationId,contactId:input.contactId,engagementId:input.engagementId},remoteConfirmed:false});let transmitted=false;
    try{const result=await this.calendarWriter.create(this.config(calendar),this.vault.read(String(calendar.credentialKey)),String(calendar.providerCalendarKey),input.event);transmitted=true;this.confirmReconciliation(reconciliationId,{remoteConfirmed:true,result});return this.applyReconciliation(reconciliationId) as any;}catch(error){if(!transmitted)this.removeReconciliation(reconciliationId);this.finishCalendarFailure(operation.id,error,transmitted);throw new Error(transmitted?'Calendar event changed remotely but local reconciliation is required. Do not repeat the operation.':error instanceof Error?error.message:String(error));}
  }
  async updateCalendarEvent(id:string,event:CalendarWriteInput){const current=this.requireEvent(id);const resourceHref=this.resourceHrefForEvent(id);const calendar=this.requireCalendar(current.calendarId);const request={...event,providerEventKey:current.providerEventKey,resourceHref,etag:current.etag};const operation=this.repository.startCalendarOperation(current.calendarId,'update',id,current.etag,request);const reconciliationId=this.createReconciliation('calendar',operation.id,'update',{eventId:id,event:request,remoteConfirmed:false});let transmitted=false;try{const result=await this.calendarWriter.update(this.config(calendar),this.vault.read(String(calendar.credentialKey)),String(calendar.providerCalendarKey),request);transmitted=true;this.confirmReconciliation(reconciliationId,{remoteConfirmed:true,result});return this.applyReconciliation(reconciliationId);}catch(error){if(!transmitted)this.removeReconciliation(reconciliationId);this.finishCalendarFailure(operation.id,error,transmitted);throw new Error(transmitted?'Calendar event changed remotely but local reconciliation is required. Do not repeat the operation.':error instanceof Error?error.message:String(error));}}
  async cancelCalendarEvent(id:string){const current=this.requireEvent(id);const resourceHref=this.resourceHrefForEvent(id);const calendar=this.requireCalendar(current.calendarId);const event:CalendarWriteInput={providerEventKey:current.providerEventKey,resourceHref,etag:current.etag,title:current.title,description:current.description,location:current.location,startsAt:current.startsAt,endsAt:current.endsAt,timezone:current.timezone,recurrence:current.recurrence,attendees:current.attendees,cancelled:true};const operation=this.repository.startCalendarOperation(current.calendarId,'cancel',id,current.etag,event);const reconciliationId=this.createReconciliation('calendar',operation.id,'cancel',{eventId:id,event,remoteConfirmed:false});let transmitted=false;try{const result=await this.calendarWriter.cancel(this.config(calendar),this.vault.read(String(calendar.credentialKey)),String(calendar.providerCalendarKey),event);transmitted=true;this.confirmReconciliation(reconciliationId,{remoteConfirmed:true,result});return this.applyReconciliation(reconciliationId);}catch(error){if(!transmitted)this.removeReconciliation(reconciliationId);this.finishCalendarFailure(operation.id,error,transmitted);throw new Error(transmitted?'Calendar event changed remotely but local reconciliation is required. Do not repeat the operation.':error instanceof Error?error.message:String(error));}}
  ...
  private confirmReconciliation(id:string,patch:Record<string,unknown>){const connection=getSqliteConnection();const row=connection.prepare('SELECT payload_json FROM outbound_reconciliation_records WHERE id=?').get(id) as {payload_json:string}|undefined;if(!row)throw new Error('Outbound reconciliation record not found');const payload=parseJson<Record<string,unknown>>(row.payload_json,{});connection.prepare('UPDATE outbound_reconciliation_records SET payload_json=?,updated_at=? WHERE id=?').run(JSON.stringify({...payload,...patch}),new Date().toISOString(),id);}
  private removeReconciliation(id:string){getSqliteConnection().prepare('DELETE FROM outbound_reconciliation_records WHERE id=?').run(id);}
  private applyReconciliation(id:string):unknown{
    const connection=getSqliteConnection();const row=connection.prepare('SELECT * FROM outbound_reconciliation_records WHERE id=?').get(id) as Record<string,unknown>|undefined;if(!row)throw new Error('Outbound reconciliation record not found');if(row.status==='reconciled')return null;const payload=parseJson<Record<string,any>>(row.payload_json,{});if(payload.remoteConfirmed!==true)throw new Error('Remote outcome is unknown. Verify the provider state before reconciling or repeating this operation.');
    let result:unknown;let afterCommit:(()=>void)|null=null;
    try{
      ...
      if(row.kind==='email'){
        ...
        const draft=this.getDraft(String(payload.draftId));result=draft;connection.prepare(`UPDATE outbound_email_attempts SET status='succeeded',provider_message_key=?,rfc_message_id=?,error_summary=NULL,completed_at=? WHERE id=(SELECT id FROM outbound_email_attempts WHERE draft_id=? ORDER BY started_at DESC LIMIT 1)`).run(payload.providerMessageKey,payload.rfcMessageId,completedAt,payload.draftId);
        if(draft.sentMessageId)afterCommit=()=>{this.triggers.trigger({triggerType:'email_sent',sourceType:'email_message',sourceId:String(draft.sentMessageId),eventId:String(draft.sentMessageId),context:{accountId:draft.accountId,organisationId:draft.organisationId,contactId:draft.contactId,engagementId:draft.engagementId,threadId:draft.threadId,subject:draft.subject}});};
      }else if(row.operation==='create'){
        const existing=connection.prepare('SELECT id FROM calendar_events WHERE calendar_id=? AND provider_event_key=?').get(payload.calendarId,payload.result.providerEventKey) as {id:string}|undefined;result=existing?this.requireEvent(existing.id):this.repository.createLocalCalendarEvent(payload.calendarId,payload.event,{...payload.context,etag:payload.result.etag});const event=result as any;this.upsertCalendarResource(String(event.id),payload.calendarId,payload.result.resourceHref);connection.prepare(`UPDATE calendar_write_operations SET status='succeeded',resulting_etag=?,provider_event_key=?,error_summary=NULL,completed_at=? WHERE id=?`).run(payload.result.etag,payload.result.providerEventKey,completedAt,row.source_id);
        afterCommit=()=>{this.triggers.trigger({triggerType:'calendar_event_created',sourceType:'calendar_event',sourceId:String(event.id),eventId:String(event.id),context:{organisationId:event.organisationId,contactId:event.contactId,engagementId:event.engagementId,title:event.title,startsAt:event.startsAt,endsAt:event.endsAt}});};
      }else{
      ...
      connection.prepare("UPDATE outbound_reconciliation_records SET status='reconciled',error_summary=NULL,reconciled_at=?,updated_at=? WHERE id=?").run(completedAt,completedAt,id);
    ...
    if(afterCommit){try{afterCommit();}catch(error){console.error('Outbound reconciliation workflow trigger failed:',error);}}
    return result;
  }

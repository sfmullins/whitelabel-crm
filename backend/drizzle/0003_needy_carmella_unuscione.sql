...
CREATE TRIGGER wi4_contacts_search_ai AFTER INSERT ON contacts BEGIN
  ...
  VALUES (
    ...
    coalesce(nullif(coalesce(nullif(coalesce(nullif(trim(coalesce(new.first_name, '') || ' ' || coalesce(new.last_name, '')), ''), nullif(trim(coalesce(new.email, '')), ''), 'Unnamed contact'), ''), nullif(trim(coalesce(new.email, '')), ''), 'Unnamed contact'), ''), nullif(trim(coalesce(new.email, '')), ''), 'Unnamed contact'),

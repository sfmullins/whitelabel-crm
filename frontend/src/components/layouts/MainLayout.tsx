...
function ActiveWorkspace({user,can}:{user:CrmIdentity;can:(permission:string)=>boolean}){
  ...
  const allGroups=extensionItems.length?[...navGroups,{label:'Extensions',items:extensionItems.sort((a,b)=>a.label.localeCompare(b.label))}]:navGroups;
  const visibleGroups=allGroups.map((group)=>({...group,items:group.items.filter(visible)})).filter((group)=>group.items.length);

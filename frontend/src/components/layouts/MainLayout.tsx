...
import {
  ...
  Layers, Link2, Plus, Radio, Search, Settings, ShieldCheck, UserRound, Users, Workflow, X,
} from 'lucide-react';
...
const navGroups = [
  ...
  { label: 'System', items: [
    { to: '/operations-health', label: 'Operations health', icon: ShieldCheck },
...
export default function MainLayout() {
  ...
  const pinnedViews = useQuery<SavedView[]>({queryKey: ['saved-views', 'pinned'],queryFn: () => api.get('/api/saved-views?pinnedOnly=true'),enabled: isSearchOpen});
  const flatResults = search.data?.items ?? [];

  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') {event.preventDefault(); setIsSearchOpen((open) => !open);}
  ...
  useEffect(() => {if (isSearchOpen) { setQuery(''); setSelectedIndex(0); requestAnimationFrame(() => inputRef.current?.focus()); }}, [isSearchOpen]);

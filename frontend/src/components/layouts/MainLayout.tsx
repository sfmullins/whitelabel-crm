...
function ActiveWorkspace({user,can}:{user:CrmIdentity;can:(permission:string)=>boolean}){
  ...
  const signOut=async()=>{await logout();client.setQueryData(['crm-identity'],{user:null});client.removeQueries();};
  if(isLoading||workspaceModel.isLoading)return <div className="flex min-h-screen items-center justify-center bg-slate-50 text-sm text-slate-500">Opening local CRM workspace…</div>;
  if(brandingError||!settings)return <LifecycleBlock title="Published settings unavailable" message={(brandingError as Error)?.message||'The active instance does not have a readable settings projection.'} onSignOut={signOut}/>;
  if(workspaceModel.isError||!workspaceModel.data)return <LifecycleBlock title="Workspace model unavailable" message={(workspaceModel.error as Error)?.message||'The active CRM model could not be loaded. Restart the local client so its database upgrade can complete.'} onSignOut={signOut}/>;

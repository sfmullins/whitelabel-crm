...
export function useProvisioningWorkspace(onSuccess?:()=>void){
  ...
  async function persistLatestDraft():Promise<OnboardingWorkspace|null>{
    if(savePromise.current){
      try{await savePromise.current;}catch{return null;}
      const currentAfterWait=stateRef.current;
      if(!currentAfterWait||JSON.stringify(currentAfterWait.draft)===lastSaved.current)return currentAfterWait?.workspace??null;
    }
    ...
    if(serialized===lastSaved.current){setSaveState('saved');return current.workspace;}
    setSaveState('saving');
    ...
    try{
      ...
      if(latest&&JSON.stringify(latest.draft)!==lastSaved.current)return persistLatestDraft();
      return workspace;
    }catch(error){setSaveState('error');setMessage(error instanceof Error?error.message:'The onboarding draft could not be saved.');return null;}
  ...
  const downloadDeploymentFiles=()=>{if(!stateRef.current?.profile)return;const profile=stateRef.current.profile;const base=profile.profile.businessIdentity.displayName.toLowerCase().replace(/[^a-z0-9]+/g,'-')||'crm';download(JSON.stringify(profile,null,2),`${base}.crmdeploy.json`,'application/json');window.setTimeout(()=>download(profile.publicKey,`${base}.crmdeploy.json.pub`,'text/plain'),120);setMessage('The signed profile and detached public-key trust anchor were downloaded. WI13 must package both.');};

...
import type { Customer,CustomFieldDefinition,CustomObjectRecord,WorkspaceModelDefinition } from 'shared';
...
export default function ModelRecords(){
  ...
  return <div className="space-y-6">
    ...
    {showCreate&&<CreateRecordDialog definition={definition} customers={customers.data??[]} customerLabel={model.data?.customerSingular??'Customer'} onClose={()=>setShowCreate(false)}/>}
  </div>;
}

function CreateRecordDialog({definition,customers,customerLabel,onClose}:{definition:WorkspaceModelDefinition;customers:Customer[];customerLabel:string;onClose:()=>void}){
  const client=useQueryClient();const [customerId,setCustomerId]=useState(customers[0]?.id??'');const [values,setValues]=useState<Record<string,string>>({});const [error,setError]=useState('');
  const create=useMutation({
    mutationFn:()=>api.post('/api/custom-objects/records',{objectDefinitionId:definition.id,customerId,values}),

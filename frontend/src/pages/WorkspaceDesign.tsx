...
function DeleteObjectDialog({definition,onClose,onDeleted}:{definition:WorkspaceModelDefinition;onClose:()=>void;onDeleted:()=>Promise<void>}){
  ...
  return <Dialog title={`Permanently delete ${definition.name}?`} description="This cannot be undone from the CRM. A database backup is the only recovery route." onClose={onClose}>{impact.isLoading?<State message="Calculating impact…"/>:<div className="space-y-4">{impact.data?.managedByExtension?<ErrorText text={`This object or one of its fields is supplied by extension package ${impact.data.managedByExtension}. Manage that package from Extensions instead.`}/>:<><div className="grid grid-cols-2 gap-3">{[['Records',impact.data?.recordCount],['Fields',impact.data?.fieldCount],['Connections',impact.data?.relationshipCount],['Saved links',impact.data?.linkedRecordCount]].map(([label,value])=><div key={String(label)} className="rounded-xl border bg-destructive/5 p-3"><p className="text-2xl font-black">{value??0}</p><p className="text-xs text-muted-foreground">{label} removed</p></div>)}</div><Field label={`Type ${definition.name} to confirm`}><Input value={confirmation} onChange={(event)=>setConfirmation(event.target.value)}/></Field>{error&&<ErrorText text={error}/>}</>}<div className="flex justify-end gap-2"><Button variant="outline" onClick={onClose}>{impact.data?.managedByExtension?'Close':'Cancel'}</Button>{!impact.data?.managedByExtension&&<Button className="bg-destructive text-destructive-foreground" disabled={confirmation!==definition.name||remove.isPending} onClick={()=>remove.mutate()}>Delete permanently</Button>}</div></div>}</Dialog>;
}
